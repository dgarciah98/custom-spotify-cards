...
use common::{model::{CardData, AccessToken}, cards::{CanvasAssets, TextAssets}};
...
pub(crate) fn card_view(props: &CardViewProps) -> Html {
...

    {
        ...
        use_effect_with(
            (track_id.clone(), canvas_assets.clone(), bg_type.clone()),
            move |_| {
                spawn_local(async move {
					let mut new_card_data = (*card_data).clone();
					if new_card_data.clone().is_none() || new_card_data.clone().unwrap().track_id != track_id {
						log::info!("fetch_data");
					...
					if new_card_data.clone() != (*card_data).clone() || (*card_data).clone().is_none() {
						...
						new_canvas_assets = Some(common::cards::generate_canvas_assets(new_card_data.clone().unwrap()));
					...
					if (*bg_type).clone() != (*prev_bg_type).clone() {
						...
						let generated_image = common::cards::generate_card(
							if !new_card_data.clone().is_none() { new_card_data.clone().unwrap() } else { (*card_data).clone().unwrap() },
							if !new_canvas_assets.clone().is_none() { new_canvas_assets.clone().unwrap() } else { (*canvas_assets).clone().unwrap() },
							if !new_text_assets.clone().is_none() { new_text_assets.clone().unwrap() } else { (*text_assets).clone().unwrap() },
							(*bg_type).clone()
						);
